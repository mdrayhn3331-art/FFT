...
function productOptions(p){
  const sizes=Array.isArray(p.available_sizes)?p.available_sizes.filter(Boolean):[];
  const colors=Array.isArray(p.available_colors)?p.available_colors.filter(Boolean):[];
  return `<div class="variant-grid">${sizes.length?`<label class="variant-field"><span>📏 Size</span><select name="size" id="productSize" required><option value="">Select Size</option>${sizes.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('')}</select></label>`:''}${colors.length?`<label class="variant-field"><span>🎨 Color</span><select name="color" id="productColor" required><option value="">Select Color</option>${colors.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('')}</select></label>`:''}</div>`;
}
function selectedVariant(){return {size:$('#productSize')?.value||'',color:$('#productColor')?.value||''}}
async function showProduct(id){const p=products.find(x=>x.id===id);if(!p)return;const user=await refreshUser();if(!user){showAuth();return}if(p.category==='Premium Apps'){const {data:plans}=await supabase.from('membership_plans').select('*').eq('is_active',true).order('price');openModal(`<h2>💎 ${p.name}</h2><p>${p.description||'Choose a premium plan.'}</p><div class="form">${(plans||[]).map(x=>`<button class="secondary btn3d plan" data-id="${x.id}">${x.name} · ${money(x.price)} · ${x.duration_days} days</button>`).join('')||'<div class="msg">No active plans yet.</div>'}</div>`);document.querySelectorAll('.plan').forEach(b=>b.onclick=()=>buyPremium(b.dataset.id));return}if(p.category==='Services'){openServiceCheckout(p);return}openModal(`<h2>🛍️ ${esc(p.name)}</h2><p>${esc(p.description||'')}</p><p><b>${money(p.price)}</b></p>${productOptions(p)}<div class="form"><button class="primary btn3d" id="add">Add to Cart</button><button class="secondary btn3d" id="cod">Buy with COD</button><button class="primary btn3d" id="bal">Pay with Balance</button></div>`);
$('#add').onclick=()=>{const v=selectedVariant();const key=`${p.id}|${v.size}|${v.color}`;const x=cart.find(x=>x.key===key);x?x.qty++:cart.push({key,id:p.id,name:p.name,price:p.price,qty:1,size:v.size,color:v.color});updateCart();closeModal()};
$('#cod').onclick=()=>checkout(p,'cod',selectedVariant());$('#bal').onclick=()=>checkout(p,'balance',selectedVariant())}
...
function addressForm(id,submitText,variant={}){const sizes=variant.sizes||[];const colors=variant.colors||[];return `<form class="form" id="${id}">${sizes.length?`<label class="variant-field"><span>📏 Size</span><select name="size" required><option value="">Select Size</option>${sizes.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('')}</select></label>`:''}${colors.length?`<label class="variant-field"><span>🎨 Color</span><select name="color" required><option value="">Select Color</option>${colors.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('')}</select></label>`:''}<input name="name" required placeholder="Full name"><input name="phone" required placeholder="Phone number"><input name="division" required placeholder="Division"><input name="district" required placeholder="District"><input name="thana" required placeholder="Thana / Upazila"><input name="area" required placeholder="Area"><textarea name="address" required placeholder="Full delivery address"></textarea><button class="primary btn3d">${submitText}</button></form>`}
async function checkout(p,method,variant={}){const sizes=Array.isArray(p.available_sizes)?p.available_sizes.filter(Boolean):[];const colors=Array.isArray(p.available_colors)?p.available_colors.filter(Boolean):[];openModal(`<h2>${method==='cod'?'🚚 Cash on Delivery':'💰 Pay with Balance'}</h2>${method==='cod'?'<div class="msg">Delivery fee may apply. Pay when the parcel arrives.</div>':''}${addressForm('orderForm',method==='cod'?'Place COD Order':'Confirm Balance Payment',{sizes:variant.size?[variant.size]:sizes,colors:variant.color?[variant.color]:colors})}`);$('#orderForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);const address=[f.get('name'),f.get('division'),f.get('district'),f.get('thana'),f.get('area'),f.get('address')].join(', ');const size=f.get('size')||variant.size||null;const color=f.get('color')||variant.color||null;const {data:orderId,error}=await supabase.rpc('fft_create_order_with_payment',{p_customer_name:f.get('name'),p_customer_phone:f.get('phone'),p_customer_address:address,p_payment_method:method,p_items:[{product_id:p.id,quantity:1,size,color}],p_shipping_fee:0});if(error)alert(error.message);else{await refreshUser();openModal(`<h2>✅ Order Placed</h2><div class="msg">Your ${method==='cod'?'COD':'balance'} order has been submitted.</div>`)}}}
...
(function(){
  ...
  async function checkoutCart(method){
    ...
    document.getElementById('cartOrderForm').onsubmit=async e=>{
      ...
      const items=cart.map(x=>({product_id:x.id,quantity:Number(x.qty)||1,size:x.size||null,color:x.color||null}));
  ...
  if(cartBtn) cartBtn.onclick=()=>{
    ...
    openModal(`<h2>🛒 Your Cart</h2><div class="form">${cart.map(x=>`<div class="wallet-card"><span>${esc(x.name)} × ${x.qty}<br><small>${x.size?`Size: ${esc(x.size)} `:''}${x.color?`Color: ${esc(x.color)}`:''}</small></span><b>${money(Number(x.price)*Number(x.qty))}</b></div>`).join('')}<div class="wallet-card"><span>Total</span><b>${money(total)}</b></div><button class="primary btn3d" id="cartCod">🚚 Checkout COD</button><button class="primary btn3d" id="cartBalance">💰 Pay with Balance</button><button class="secondary btn3d" id="clearCart">Clear Cart</button></div>`);
